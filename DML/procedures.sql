-- PROCEDURE: public.gestion_ganadores(integer, integer)

-- DROP PROCEDURE IF EXISTS public.gestion_ganadores(integer, integer);

CREATE OR REPLACE PROCEDURE public.gestion_ganadores(
	v_ano_oscar integer,
...
begin
	...
	if not found then
		RAISE EXCEPTION 'No se han introducido ni  dentro de esta categoria';
	end if;
	----Comprobar Empate
	...
	if(v_length_temp> 1) then 
		...
		---------------------------Notificar Usuario--------------------------
		v_message= concat(v_message,'. Los postulados han sido nuevamente ingresados en  para una votacion de desempate');
	...
	select id_nominada into v_id_ganador from tmp_votos;

    raise notice 'El ganador ha sido la nominacion numero: %',v_id_ganador;
	drop table tmp_votos;
...
>>>>>>> f0537a65c143a0017b1ae0e2b86d31f3cbbc69a0
 -------Prueba---------
 --call validar_actor();
